...
- write Markdown Confusion Matrix to file
...
int append_image_data_csv(const char *filename, const char *image_filename, std::vector<float> &image_data, int reset_file)
{
  ...
  {
    char tmp[256];
    // formats the number as a string with a comma prefix
...
/*
  ...
  The function returns a non-zero value in case of an error.
 */
int append_labeled_image_data_csv(const char *filename, const char *label, const char *image_filename, std::vector<float> &image_data, int reset_file)
{
  char buffer[256];
  char mode[8];
  FILE *fp;
  ...
  // write the filename,label and the feature vector to the CSV file
  fprintf(fp, "%s,%s", image_filename, label); // Use fprintf for formatting
...
/*
  ...
  The function returns a non-zero value if something goes wrong.
 */
int read_labeled_image_data_csv(const char *filename, std::vector<char *> &filenames, std::vector<char *> &labels, std::vector<std::vector<float>> &data, int echo_file)
{
  FILE *fp;
  float fval;
  char img_file[256];
  ...
  for (;;)
  {
    ...
    char *lbl = new char[strlen(label) + 1];
    strcpy(lbl, label);
...
/**
 * @brief 
 * write to any stream output
 * (std::cout) print to console, (std::ofstream) to specific file
 * @param out std::cout print to console, std::ofstream to specific file
 * @param actual_predict_counts_map 
 */
void writeConfusionMatrix(std::ostream& out, std::map<std::string, std::map<std::string, int>> &actual_predict_counts_map) {
   std::set<std::string> labels;
    
    // Collect all labels
    for (const auto& actualPair : actual_predict_counts_map) {
        labels.insert(actualPair.first);
        for (const auto& predictedPair : actualPair.second) {
            labels.insert(predictedPair.first);
        }
    }
    ...
    // adding out stream by reference to lambda capture list
    std::for_each(labels.begin(), labels.end(), [&out](const std::string& label) {
        out << " " << label << " |";
    });
    out << std::endl;
    ...
    // Print separator row
    out << "|----";
    std::fill_n(std::ostream_iterator<std::string>(out), labels.size(), "|----");
    out << "|" << std::endl;
    
    // Print data rows
    for (const auto& actualLabel : labels) {
        out << "| " << actualLabel << " |";
        std::for_each(labels.begin(), labels.end(), [&](const std::string& predictedLabel) {
            auto itActual = actual_predict_counts_map.find(actualLabel);
            if (itActual != actual_predict_counts_map.end()) {
        
                auto itPredicted = itActual->second.find(predictedLabel);
                // cell is 0 if Predicted label not found for actual label
                out << " " << (itPredicted != itActual->second.end() ? itPredicted->second : 0) << " |";
            } else {
                out << " 0 |"; // Actual label not found
            }
        });
        out << std::endl;
    }
